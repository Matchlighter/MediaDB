...
(function($) {
  $.fn.formset = function(opts) {
    ...
    if ($this.length && showAddButton) {
      ...
      addButton.click(function(e) {
        ...
        var template = $("#" + options.prefix + "-__prefix__");
        var row = template.clone(true);
        row.removeClass(options.emptyCssClass)
          .addClass(options.formCssClass)
          .attr("id", options.prefix + "-" + nextIndex);
		
		var $deleteBtn = options.getDeleteBtn(row, options);
  ...
  $.fn.formset.defaults = {
	...
	deleteHTML: '<span><a class="{deleteClass}" href="javascript:void(0)">{deleteText}</a></span>',
	getDeleteBtn: function (row, options) { //Gets the delete button for a dynamic form.
        ...
        if (row.is("tr")) {
          ...
          row.children(":first").append(delhtml);
        }
  ...
  $.fn.tabularFormset = function(options) {
    ...
    $rows.formset({
	  ...
	  deleteHTML: '<a class="{deleteClass}" href="javascript:void(0)">{deleteText}</a>',
  ...
  $.fn.stackedFormset = function(options) {
    ...
    var reinitDateTimeShortCuts = function() { //Same
    ...
    var updateSelectFilter = function() { //Same
    ...
    var initPrepopulatedFields = function(row) { //Slight diff (.form-row)
      row.find('.prepopulated_field').each(function() {
        var field = $(this),
            input = field.find('input, select, textarea'),
            dependency_list = input.data('dependency_list') || [],
    ...
    $rows.formset({
      ...
      deleteCssClass: "inline-deletelink",
      deleteText: options.deleteText,
	  deleteHTML: '<li><a class="{deleteClass}" href="javascript:void(0)">{deleteText}</a></li>',
      emptyCssClass: "empty-form",
